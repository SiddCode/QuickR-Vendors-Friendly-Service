      ...
      {!isStaff && (
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-soft space-y-5 font-sans">
          <div className="flex items-center justify-between">
            ...
            {trendsResult ? (
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
              >
          ...
          {!trendsResult && !trendsLoading && !trendsError && (
            <div className="text-center py-6 bg-slate-50/50 rounded-xl border border-slate-100 space-y-3">
              <p className="text-xs text-slate-500 font-medium max-w-md mx-auto">
                Compare business performance over the last 30 days against the previous period with AI analysis.
              </p>
              <button
                onClick={async () => {
                  setTrendsLoading(true);
          ...
          {trendsLoading && (
            <div className="py-8 text-center space-y-2 bg-slate-50/50 rounded-xl border border-slate-100">
              ...
              <p className="text-xs text-slate-400">Comparing 30-day periods across sales, revenue, enquiries & conversion rates</p>
            </div>
          )}
          ...
          {trendsResult && !trendsLoading && (
            <div className="space-y-5 animate-fadeIn">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  { label: 'Revenue Trend', curr: `₹${(trendsResult.metrics?.currentPeriod?.revenue || 0).toLocaleString('en-IN')}`, prev: `₹${(trendsResult.metrics?.previousPeriod?.revenue || 0).toLocaleString('en-IN')}`, pct: trendsResult.metrics?.changes?.revenuePctChange },
                  { label: 'Sales Count', curr: `${trendsResult.metrics?.currentPeriod?.salesCount || 0} sales`, prev: `${trendsResult.metrics?.previousPeriod?.salesCount || 0} sales`, pct: trendsResult.metrics?.changes?.salesCountPctChange },
                  { label: 'Enquiries', curr: `${trendsResult.metrics?.currentPeriod?.enquiriesCount || 0} enquiries`, prev: `${trendsResult.metrics?.previousPeriod?.enquiriesCount || 0} enquiries`, pct: trendsResult.metrics?.changes?.enquiriesPctChange },
                  { label: 'Conversion Rate', curr: `${trendsResult.metrics?.currentPeriod?.conversionRate || 0}%`, prev: `${trendsResult.metrics?.previousPeriod?.conversionRate || 0}%`, pct: trendsResult.metrics?.changes?.conversionRateDiff, isDiff: true }
                ].map((m, idx) => (
                  <div key={idx} className="p-3.5 bg-slate-50/70 rounded-xl border border-slate-100 text-left space-y-1">
                    <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">{m.label}</span>
                    <p className="text-sm font-black text-slate-800">{m.curr}</p>
                    <div className="flex items-center justify-between text-[11px] pt-1">
                      <span className="text-slate-400 font-medium">Prev: {m.prev}</span>
                      {m.pct !== undefined && m.pct !== null ? (
                        <span className={`font-bold flex items-center gap-0.5 ${m.pct > 0 ? 'text-emerald-600' : m.pct < 0 ? 'text-rose-600' : 'text-slate-500'}`}>
                          {m.pct > 0 ? <TrendingUp className="w-3 h-3" /> : m.pct < 0 ? <TrendingDown className="w-3 h-3" /> : <Minus className="w-3 h-3" />}
                          {m.pct > 0 ? '+' : ''}{m.pct}{m.isDiff ? '%' : '%'}
                        </span>
                      ) : null}
                    </div>
                  </div>
              ...
              {trendsResult.aiInsights && (
                <div className="space-y-4 pt-2 border-t border-slate-100">
                  <div className="p-4 bg-emerald-50/60 border border-emerald-100 rounded-xl">
                    <span className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider block mb-1">Trend Analysis Summary</span>
                    <p className="text-xs font-semibold text-slate-800 leading-relaxed">{trendsResult.aiInsights.summary}</p>
                  </div>

                  <div>
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Key Performance Trends</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {trendsResult.aiInsights.trends.map((t, idx) => (
                        <div key={idx} className="p-4 bg-slate-50/70 rounded-xl border border-slate-100 space-y-1 text-left">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">{t.type}</span>
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${t.importance === 'HIGH' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>
                              {t.importance}
                            </span>
                          </div>
